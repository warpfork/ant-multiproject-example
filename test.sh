...
function testProject {(
	...
	echo -n "project '$PROJECT' did one-time setup once    ...   " ; [ 1 -eq `echo "$output" | grep "hullo, performing one-time setup.$" | wc -l` ] && echo "PASS" || echo "FAIL"
	echo -n "project '$PROJECT' used macro repeatedly      ...   " ; [ 1 -lt `echo "$output" | grep "hullo, macrodef inherited.$" | wc -l` ] && echo "PASS" || echo "FAIL"
	echo ====
	echo
)}
